...
# Iniciar al cargar el modulo - funciona con gunicorn
print("[MILO] Iniciando - obteniendo token...")
_t_inicio = threading.Thread(target=renovar_token, daemon=True)
_t_inicio.start()
_tr = threading.Thread(target=hilo_renovador, daemon=True)
_tr.start()
...
def health():
    token = leer_token()
    return jsonify({"status": "OK", "token": bool(token), "preview": token[:25]+"..." if token else "SIN TOKEN"})
